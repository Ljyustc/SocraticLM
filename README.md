...
- [`SocraTeach_single.json`](data/SocraTeach_single.json) is a dataset containing 22K single-round "Teacher-Student" teaching dialogues. This dataset contains four categories of keys: "irrelevant," "question," "incorrect," and "correct," which correspond to four types of student responses. In each dialogue, the "history" refers to the preceding conversation, the "prompt" represents the augmented student reply in the current round, and the "response" denotes the expected reply from the teacher.
